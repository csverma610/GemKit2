...
class GeminiVideoAnalyzer:
    ...
    def upload_file(self, file_path: str, timeout: int = 300, check_interval: int = 5) -> Optional[genai.File]:
        """
        Uploads a local video file to the Gemini API and waits for it to be processed.
        :param file_path: The local path to the video file (e.g., 'path/to/video.mp4').
        :param timeout: Maximum time to wait for processing in seconds (default: 300s = 5 minutes)
        :param check_interval: Time between status checks in seconds (default: 5s)
        :return: The uploaded file object (genai.File) from the API or None on error.
        """
        if not self.client:
    ...
    def generate_text(self, prompt: str = "Describe the video") -> Optional[str]:
        """
        Generates content based on the uploaded video and prompt.
        :param prompt: The text prompt for analysis (default: "Describe the video")
        :return: Generated text or None on error
        """
        if not self.client:
    ...
    def delete_file(self) -> None:
        """
        Deletes the file from the Gemini API service after processing. 
        This is crucial for cleanup and managing storage quotas.
        """
